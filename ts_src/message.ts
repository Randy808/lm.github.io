...
function getMessage(
  ...
  let lastRingPositionInSignatureBuffer =
    (lastRingPosition + lastIndexOfRing) * SIGNATURE_SIZE;
  ...
  for (let ringIndex = 0; ringIndex < numRings; ringIndex++) {
    for (let sigIndex = 0; sigIndex < ringSize; sigIndex++) {
      ...
      if (secidx[ringIndex] === sigIndex) {
        fakeSignatures += Buffer.from(
          new Array(SIGNATURE_SIZE).fill(0)
        ).toString("hex");
        continue;
      }
...
export function getMessageFromDecryptedSignature(
  numRings: number,
  ringSize: number,
  decryptedSignatureData: Buffer
) {
  ...
  let secondToLastValueBytes = decryptedSignatureData.subarray(
    startIndex - 8,
    endIndex - 8
  );
  ...
  let secidx = getSecIdx(BigInt("0x" + valueHex));
  let decryptedMessage: Buffer | undefined;
  ...
  for (let ringIndex = 0; ringIndex < numRings; ringIndex++) {
    for (let sigIndex = 0; sigIndex < ringSize; sigIndex++) {
      ...
      let startOfMessageChunk =
        ringIndex * ringSize * SIGNATURE_SIZE + sigIndex * SIGNATURE_SIZE;
      let endOfMessageChunk = startOfMessageChunk + SIGNATURE_SIZE;
      let message = decryptedSignatureData.subarray(
        startOfMessageChunk,
        endOfMessageChunk
      );
