...
function decryptByteValues(signatures, decryptionKeys) {
    let decryptedByteValues = [];
...
function main(nonce, t, outputIndex, message) {
    //reset
    const index = outputIndex;
    function convertParityByteToQuadness(bytes) {
        if (bytes.length === 0) {
    ...
    let decryptionKeys = genrand(nonce, serializedPoint, serializedGenP, new Uint8Array(Array(3328).fill(0)));
    let signatures = Buffer.from(t.outs[index].rangeProof.subarray(846).toString("hex"), "hex");
    let commitments = Buffer.from(t.outs[index].rangeProof.subarray(14, 814).toString("hex"), "hex");
    let sharedRootMessageHash = Buffer.from(t.outs[index].rangeProof.subarray(814, 814 + 32).toString("hex"), "hex");
    let { decryptedByteValues } = decryptByteValues(signatures, decryptionKeys);
    console.log("\n\nDECRYPTED\n\n", Buffer.from(decryptedByteValues).toString("hex"));
    let valueBigIntArg = BigInt("0x" +
        Buffer.from(decryptedByteValues.slice(decryptedByteValues.length - 8)).toString("hex"));
    ...
    let { k, es, sec } = generateRangeProof2(serializedPoint, serializedGenP, nonce, valueBigIntArg, extraCommitBuffer, assetIdHex, assetBlind, sharedRootMessageHash, genP);
    const lastRing = 25;
    let ind = secidx[lastRing];
    let realSigForLastRing = signatures.subarray(lastRing * 4 * 32 + ind * 32, lastRing * 4 * 32 + ind * 32 + 32);
    let e_i_inverse = invert(es[es.length - 1], Fn.ORDER);
    let base = Fn.create(Fn.create(Fn.fromBytes(k[lastRing]) - Fn.fromBytes(realSigForLastRing)) *
        e_i_inverse);
    let ephemeralOutputBlind = Fn.create(base - Fn.fromBytes(sec[lastRing]));
    let { finalProof: rangeProof } = generateRangeProof(serializedPoint, serializedGenP, ephemeralOutputBlind, nonce, valueBigIntArg, extraCommitBuffer, assetIdHex, assetBlind, genP, message);
    console.log();
...
function getDecryptedRingSignatureRangeProof(nonce, t, outputIndex) {
    //reset
    const index = outputIndex;
    return getDecryptedRingSignatureRangeProofFromOutput(nonce, t.outs[outputIndex]);
}
function getDecryptedRingSignatureRangeProofFromOutput(nonce, o) {
    function convertParityByteToQuadness(bytes) {
        if (bytes.length === 0) {
            throw new Error("Bytes cannot be zero-length");
        }
    ...
    let decryptionKeys = genrand(nonce, serializedPoint, serializedGenP, new Uint8Array(Array(3328).fill(0)));
    let signatures = Buffer.from(o.rangeProof.subarray(846).toString("hex"), "hex");
    let { decryptedByteValues } = decryptByteValues(signatures, decryptionKeys);
