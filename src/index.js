...
import { getDecryptedRingSignatureRangeProof, getDecryptedRingSignatureRangeProofFromOutput, modifyRangeProof, } from "./modify-tx-with-rangeproof";
...
function selectUTXOs(utxos, targetValue) {
    ...
    if (totalValue < targetValue) {
        throw new Error(`Insufficient funds: need ${targetValue} satoshis, but only have ${totalValue} satoshis`);
    }
...
function initializeKeypairs() {
    ...
    if (!localStorage.getItem("blindingKey")) {
        ...
        localStorage.setItem("blindingKey", blindingKeyPair.privateKey.toString("hex"));
    }
}
initializeKeypairs();
function getAddress() {
...
function getConfidentialAddress() {
    let confidentialAddress = liquidjs.address.toConfidential(getPayment().address, getBlindingKey().publicKey);
    // prompt(confidentialAddress);
    return confidentialAddress;
}
async function createBlindedTransaction(unconfidentialAddress, amount) {
    const psbt = new Psbt({
        network: NETWORK,
    });
    ...
    psbt.addOutput({
        script: payment.output,
        value: liquidjs.confidential.satoshiToConfidentialValue(TOTAL_VALUE - amount - FEE),
...
function getBlindingDataFromAddress(confidentialAddr) {
    ...
    return {
        ...
        unconfidentialAddress: address.toBech32(decoded.data.subarray(2), decoded.version, NETWORK.bech32),
    };
}
function getNonce(nonceCommitmentHex, blindingKey) {
    const ecdhNoncePreimage = sha256(secp256k1.Point.fromHex(nonceCommitmentHex)
        .multiply(Fn.create(blindingKey))
        .toBytes());
    return BigInt("0x" + Buffer.from(sha256(ecdhNoncePreimage)).toString("hex"));
}
async function showMessages(outputIndex) {
...
function getValueFromConfidentialOutput(t) {
    ...
    let verificationNonce = getNonce(nonceCommitmentHex, BigInt("0x" + getBlindingKey().privateKey.toString("hex")));
    let decrypted = getDecryptedRingSignatureRangeProofFromOutput(verificationNonce, t.outs[OUTPUT_INDEX]);
    let valueBigIntArg = BigInt("0x" + Buffer.from(decrypted.slice(decrypted.length - 8)).toString("hex"));
...
async function showTxMessage(txs, txid, outputIndex) {
    ...
    if (receiverConfidentialAddress) {
        ...
        verificationNonce = getNonce(blindingData.blindingPubKey.toString("hex"), BigInt("0x" + getBlindingKey().privateKey.toString("hex")));
    }
    else {
        blindingPublicKeyHex = nonceCommitmentHex;
        verificationNonce = getNonce(nonceCommitmentHex, BigInt("0x" + getBlindingKey().privateKey.toString("hex")));
    }
    ...
    let confidentialAddress = liquidjs.address.toConfidential(txMetadata.vin[0].prevout.scriptpubkey_address, txFinal?.outs[outputIndex]?.nonce);
    let isMine = !!receiverConfidentialAddress;
    let addressKey = isMine ? receiverConfidentialAddress : confidentialAddress;
    if (!txs[addressKey]) {
        txs[addressKey] = [];
    }
    let clientKey = messageComponents[1];
    let clientEncryptedMessage = messageComponents[2];
...
async function sendBitcoin(confidentialAddress, message) {
    ...
    let verificationNonce = getNonce(recipientBlindingData.blindingPubKey.toString("hex"), BigInt("0x" + getBlindingKey().privateKey.toString("hex")) //BigInt("0x" + recipientBlindingData.blindingPubKey.toString("hex"))
    );
    debugger;
    let { salt, clientEncryptedMessage } = encryptMessage(message);
    t = modifyRangeProof(verificationNonce, t, OUTPUT_INDEX, `lm:${salt}:${clientEncryptedMessage}`);
...
//3001 is lqd api
// sendBitcoin();
let recipientBlindingPrivateKey = 0xfff68d254e89c7aeed25b02778e31778641802d426e256dbd703f2dfd932a45an;
// showMessages(0);
