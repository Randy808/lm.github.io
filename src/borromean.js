...
import { concatBytes, numberToBytesBE, randomBytes, } from "@noble/curves/utils.js";
import { sha256 } from "@noble/hashes/sha2";
import { G, toBytes, Fn, Fp, generatePublicKeySignature, signPartOne as signFirstRoundForRing, generatePublicKeysForRing } from "./utils";
// let ringPubkeyCollection: Uint8Array[][] = Array(NUMBER_OF_RINGS)
//   .fill(undefined)
//   .map(() =>
//     generatePublicKeysForRing(RING_SIZE, signerIndex, signerPublicKey)
//   );
function getArrayWithRandomByteValues(size) {
    return Array(size)
        .fill(undefined)
...
let pubs = Array(NUMBER_OF_RINGS)
    .fill(undefined)
    .map((_val, index) => generatePublicKeysForRing(rsizes[index], secidx[index], secp256k1.getPublicKey(sec[index])));
...
export function secp256k1_borromean_sign(s, pubs, k, sec, secidx, nrings, m, skipSig = false) {
    let es = [];
    const lastRingNonceCollection = [];
    for (let ringIndex = 0; ringIndex < nrings; ringIndex++) {
        ...
        let { lastRingNonce } = signFirstRoundForRing(signerNoncePoint.toBytes(), m, ringIndex, secidx[ringIndex], pubkeys, s);
    ...
    for (let ringIndex = 0; ringIndex < nrings; ringIndex++) {
        ...
        let e_i = Fn.fromBytes(sha256(concatBytes(toBytes(sharedRootMessageHash), m, numberToBytesBE(ringIndex, 4), numberToBytesBE(0, 4))));
        // Fill in signatures from 0 to signer's index
        for (let pubkeyIndex = 0; pubkeyIndex < signerIndex; pubkeyIndex++) {
            let pubkeys = pubs[ringIndex];
            let { noncePoint } = generatePublicKeySignature(pubkeys[pubkeyIndex], e_i, s[ringIndex][pubkeyIndex]);
            // The message hash preimage format differs from the paper here,
            // and instead follows the secp256k1-zkp library implementation
            e_i = Fn.fromBytes(sha256(concatBytes(noncePoint.toBytes(), m, numberToBytesBE(ringIndex, 4), numberToBytesBE(pubkeyIndex + 1, 4))));
        }
        es.push(e_i);
        /* sig/sec[ringIndex]
        s = k - e*sec
        ...
        */
        ...
        //   }
        if (!skipSig) {
            let signerSignature = Fp.fromBytes(k[ringIndex]) -
...
export function secp256k1_borromean_sign2(s, pubs, k, sec, secidx, nrings, m, sharedRootMessageHash) {
    let es = [];
    for (let ringIndex = 0; ringIndex < nrings; ringIndex++) {
        let signerIndex = secidx[ringIndex];
        let e_i = Fn.fromBytes(sha256(concatBytes(sharedRootMessageHash, m, numberToBytesBE(ringIndex, 4), numberToBytesBE(0, 4))));
        // Fill in signatures from 0 to signer's index
        for (let pubkeyIndex = 0; pubkeyIndex < signerIndex; pubkeyIndex++) {
            let pubkeys = pubs[ringIndex];
            let { noncePoint } = generatePublicKeySignature(pubkeys[pubkeyIndex], e_i, s[ringIndex][pubkeyIndex]);
