...
import { secp256k1_borromean_sign } from "./borromean";
...
proofHeader[1] = 0x33;
proofHeader[9] = 0x01;
export function generateRangeProof(serializedPoint, serializedGenP, ephemeralOutputBlind, nonce, valueB, extraCommit, assetId, assetBlind, genP, messageText) {
    let pubs = [];
    let hmacKey = Buffer.concat([
        toBytesFn(nonce),
        serializedPoint,
    ...
    let message = getMessage(NUM_RINGS, STANDRAD_RING_SIZE, assetId, assetBlind, valueHex, secidx[LAST_RING_INDEX] === STANDRAD_RING_SIZE - 1, messageText);
    ...
    let { sharedRootMessageHash: e0 } = secp256k1_borromean_sign(sigs, pubs, k, sec, secidx, NUM_RINGS, messageHashForSignature);
    console.log();
